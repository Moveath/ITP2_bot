...
class AdaptiveTest:
    def __init__(self, source_text: str, adapt_step: int,
                 llm_generator: LLMGenerator, csv_manager: CSVManager = None):
        self.source_text = source_text
        self.adapt_step = adapt_step
        self.llm = llm_generator
        self.csv = csv_manager or CSVManager("session.csv")
        self.current_level = 1          # бастапқы деңгей
        self.answers_buffer = []        # adapt_step бойынша жинау

    def generate_next_questions(self, count: int = 1) -> list[Question]:
    ...
    def get_final_report(self) -> str:
        ...
        if not all_data:
            return "Талдау үшін деректер жоқ."
        return self.llm.generate_final_report(all_data)